...
if ($_POST) {
    ...
    for ($i = 0; $i < $numArrays; $i++) {
        ...
        if ($respuesta['error'] !== "Error" && $respuesta1['error'] !== "Error" && $respuesta2['error'] !== "Error" && $respuesta3['error'] !== "Error" && $respuesta4['error'] !== "Error") {
            ...
            $fecha0 = $fecha_salida[$i] . " "  . $hora_salida[$i];
            $fecha0 = strtotime($fecha0);

            if ($fecha0 > $fecha_hora_actual){
            $respuesta = [
                'error' => "Éxito",
            ...
            $respuesta = [
                'error' => "Error",
                'respuesta' => "La fecha salida es menor que la fecha actual"
            ];
        }
        } else {
            $respuesta = [
...
<div class="form-crud">
    <form action="alta-horario-recogida.php" method="post" class='form-alta-horario'>
        ...
        <select name="id_camioneta[]" class="estilo-select">
            <?php
            include("../../modelos/db.php");
            $instruccion = "select * from mostrar_camionetas where estado != 'De baja' and estado != 'Fuera de servicio'";
            $camionetas = [];
        ...
        <select name="id_almacen_cliente[]" class="estilo-select select-uno select-almacen">
        ...
        <div class='div-btn-doble'>
            <div class='estilo-boton btn-agregar-almacen-cliente boton-agregar' style='text-align: center; font-size: 16px;'>Agregar almacén cliente</div>
            <div class='estilo-boton btn-remover-almacen-cliente boton-volver' style='text-align: center; font-size: 16px;'>Remover almacén cliente</div>
        </div>
            <hr>
