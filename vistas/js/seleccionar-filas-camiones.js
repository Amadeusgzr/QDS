...
function enviarDatos() {
    ...
    var tabla = document.getElementById("tabla-admin-camioneros");
        ...
        if(fila.classList.contains("fila-seleccionada")){
            ...
            id_camiones.push(contenido);
        }
        

    }
    var jsonString = JSON.stringify(id_camiones);

    // Crear un objeto FormData para enviar datos al servidor
    var formData = new FormData();
    formData.append('id_camiones', jsonString);
    ...
    xhr.onload = function () {
        if (xhr.status === 200) {
            ...
            location.href = "op-camiones.php?datos=" + respuesta;
